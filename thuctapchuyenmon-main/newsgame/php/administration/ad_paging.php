    ...
    <body>
        ...
        <div>
            ...
            while ($row = mysqli_fetch_assoc($result)){
                echo '<li>' . $row['id_news'] . ' - ' . $row['title_news'] . '</li>';
            }
            ?>
        </div>
        <div class="pagination">
           <?php 
            ...
            if ($current_page > 1 && $total_page > 1){
                echo '<a href="ad_paging.php?page='.($current_page-1).'">Prev</a> | ';
            }
            ...
            for ($i = 1; $i <= $total_page; $i++){
                ...
                if ($i == $current_page){
                    ...
                    echo '<a href="ad_paging.php?page='.$i.'">'.$i.'</a> | ';
                }
            }
 
            // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
            if ($current_page < $total_page && $total_page > 1){
                echo '<a href="ad_paging.php?page='.($current_page+1).'">Next</a> | ';
            }
           ?>
        </div>
    </body>
